...
REPLAY_PATH="$OPD_REPLAY_SMOKE" SFT_CKPT="$SFT_SMOKE" OUT_PATH="$PRECHECK_SMOKE" MIN_RECOVERY_IMPROVEMENT=0 MIN_RELIABLE_POSITIVE_RATE=0.05 NPROC_PER_NODE=8 bash Trace/scripts/train/precheck_opd_teacher.sh
```
...
REPLAY_PATH="$OPD_REPLAY_SMOKE" STUDENT_CKPT="$SFT_SMOKE" TEACHER_CACHE="$PRECHECK_SMOKE" OUT_DIR="$OPD_SMOKE" MAX_SAMPLES=3 EPOCHS=1 BATCH_SIZE=1 GRAD_ACCUM=1 NUM_WORKERS=0 SAVE_STEPS=1 CLEAN=1 NPROC_PER_NODE=8 bash Trace/scripts/train/opd.sh
```
