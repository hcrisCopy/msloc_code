...
REPLAY_PATH="$OPD_REPLAY_SMOKE" SFT_CKPT="$SFT_CKPT" OUT_PATH="$PRECHECK_SMOKE" MIN_RECOVERY_IMPROVEMENT=0 MIN_RELIABLE_POSITIVE_RATE=0.05 NPROC_PER_NODE=8 bash Trace/scripts/train/precheck_opd_teacher.sh
```
...
REPLAY_PATH="$OPD_REPLAY_SMOKE" STUDENT_CKPT="$SFT_CKPT" TEACHER_CACHE="$PRECHECK_SMOKE" OUT_DIR="$OPD_SMOKE" MAX_SAMPLES=3 EPOCHS=1 BATCH_SIZE=1 GRAD_ACCUM=1 NUM_WORKERS=0 SAVE_STEPS=1 CLEAN=1 NPROC_PER_NODE=8 bash Trace/scripts/train/opd.sh
```
...
REPLAY_PATH="$GRPO_REPLAY_SMOKE" OPD_CKPT="$OPD_CKPT" OUT_DIR="$GRPO_LEXICAL_SMOKE" TEXT_REWARD_MODE=lexical EXPLANATION_WEIGHT=0.3 MAX_SAMPLES=3 GROUP_SIZE=2 MAX_NEW_TOKENS=64 EPOCHS=1 BATCH_SIZE=1 GRAD_ACCUM=1 NUM_WORKERS=0 SAVE_STEPS=1 CLEAN=1 NPROC_PER_NODE=8 bash Trace/scripts/train/grpo.sh
```
...
REPLAY_PATH="$GRPO_REPLAY_SMOKE" OPD_CKPT="$OPD_CKPT" OUT_DIR="$GRPO_NLI_SMOKE" TEXT_REWARD_MODE=nli NLI_MODEL_PATH="$NLI_MODEL_PATH" NLI_DEVICE=cuda EXPLANATION_WEIGHT=0.3 MAX_SAMPLES=3 GROUP_SIZE=2 MAX_NEW_TOKENS=64 EPOCHS=1 BATCH_SIZE=1 GRAD_ACCUM=1 NUM_WORKERS=0 SAVE_STEPS=1 CLEAN=1 NPROC_PER_NODE=8 bash Trace/scripts/train/grpo.sh
```
