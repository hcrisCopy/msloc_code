#!/bin/bash
# Train Trace in `loc` mode (no DeMamba proposals).
# All paths are relative to the MSLoc repo. Override via env vars if needed.
set -e
...
ASCEND_LAUNCH_BLOCKING=1 torchrun --nnodes $WORLD_SIZE \
    ...
    --data_path "$DATA_ROOT/annos/train_all_1209_fmt.json" \
    ...
    --tf32 False \
    --fp16 False \
    --output_dir "${OUTP_DIR}/${WANDB_PROJECT}/loc" \
    --num_train_epochs 2 \
