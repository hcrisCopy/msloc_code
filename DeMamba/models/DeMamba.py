from transformers import AutoModel, XCLIPVisionModel
import json
import os
import sys
import numpy as np
...
class DINOv3_NeuronDeMamba(nn.Module):
    ...
    The only new component is the encoder.  DINOv3's 12 blocks each contribute
    64 selected patch-token channels, preserving the original [B,T,196,768]
    tensor fed directly to Mamba; no projection, adapter, or fusion is added.
    """

    def __init__(self, neuron_indices_path, dinov3_repo_path=None, dinov3_weights_path=None,
        ...
        if dinov3_model_name != "dinov3_vitb16":
            raise ValueError("The no-adapter path is fixed to DINOv3 ViT-B/16 (dinov3_vitb16).")
        ...
        if self.dinov3_backend == "huggingface":
            ...
            if not os.path.isdir(hf_model_path):
                raise FileNotFoundError(
                    "Local Hugging Face DINOv3 directory not found: "
                    f"{hf_model_path}. Download facebook/dinov3-vitb16-pretrain-lvd1689m first."
                )
            try:
                ...
                raise RuntimeError(
                    "Unable to load Hugging Face DINOv3. Install transformers>=4.56.0 and make sure the "
                    "directory contains config.json and model.safetensors."
                ) from error
            self.hidden_size = int(getattr(self.encoder.config, "hidden_size", 0))
            self.patch_size = int(getattr(self.encoder.config, "patch_size", 0))
            self.num_hidden_layers = int(getattr(self.encoder.config, "num_hidden_layers", 0))
            ...
            if not os.path.isdir(repo_path):
                raise FileNotFoundError(f"Local DINOv3 repository not found: {repo_path}")
            if not os.path.isfile(weights_path):
                raise FileNotFoundError(f"Local DINOv3 weights not found: {weights_path}")
            self.encoder = torch.hub.load(
                repo_or_dir=repo_path, model=dinov3_model_name, source="local", weights=weights_path
            )
            self.hidden_size = int(getattr(self.encoder, "embed_dim", 0))
            self.patch_size = int(getattr(self.encoder, "patch_size", 0))
            self.num_hidden_layers = int(getattr(self.encoder, "n_blocks", 0))
            self.num_register_tokens = int(getattr(self.encoder, "n_storage_tokens", 4))
        else:
            raise ValueError("dinov3_backend must be either 'huggingface' or 'official'.")
        self.encoder.eval()
        self.patch_nums = 196
        if (self.hidden_size, self.patch_size, self.num_hidden_layers, self.num_register_tokens) != (768, 16, 12, 4):
            raise ValueError(
                "Expected DINOv3 ViT-B/16 (12 blocks, width 768, patch 16, 4 register tokens), got "
                f"blocks={self.num_hidden_layers}, width={self.hidden_size}, patch={self.patch_size}, "
        ...
        with open(selector_path, "r", encoding="utf-8") as handle:
            selector = json.load(handle)
        if selector.get("backbone") not in (None, "dinov3_vitb16"):
            raise ValueError(f"Selector was not made for DINOv3 ViT-B/16: {selector.get('backbone')!r}")
        layers = selector.get("layers", selector.get("selected_indices", selector))
        if not isinstance(layers, dict):
            raise ValueError(f"Invalid selector file: {selector_path}")

        self.selected_layer_numbers, selected_width = [], 0
        for raw_layer, raw_indices in sorted(layers.items(), key=lambda item: self._selector_layer_number(item[0])):
            layer_number = self._selector_layer_number(raw_layer)
            indices = torch.as_tensor(raw_indices, dtype=torch.long)
            if layer_number < 1 or layer_number > 12 or indices.ndim != 1 or indices.numel() == 0:
                raise ValueError(f"Invalid DINOv3 selector for layer {raw_layer!r}")
            if int(indices.min()) < 0 or int(indices.max()) >= 768 or indices.unique().numel() != indices.numel():
                raise ValueError(f"Invalid DINOv3 channel indices for layer {raw_layer!r}")
            self.register_buffer(f"neuron_indices_l{layer_number}", indices, persistent=True)
            self.selected_layer_numbers.append(layer_number)
            selected_width += int(indices.numel())
        if self.selected_layer_numbers != list(range(1, 13)) or selected_width != 768:
            raise ValueError("DINOv3 selector must contain 64 valid channels for each of 12 blocks (768 total).")

        self.mamba_configs = MambaConfig(d_model=768)
        self.mamba = ResidualBlock(config=self.mamba_configs)
        self.fc_norm = nn.LayerNorm(self.patch_nums * 768)
        self.fc_norm2 = nn.LayerNorm(768)
        self.fc1 = nn.Linear((self.patch_nums + 1) * 768, class_num)
        self.dropout = nn.Dropout(p=0.0)
        self.initialize_weights(self.fc1)
    ...
    def _selector_layer_number(name):
        text = str(name)
        return int(text[len("layer_"):] if text.startswith("layer_") else text)
    ...
    def _selected_patch_features(self, images, batch_size, num_frames):
        if self.dinov3_backend == "huggingface":
            # HF returns embedding output followed by the 12 post-block states;
            # retain only the latter and remove [CLS] plus the four registers.
            outputs = self.encoder(pixel_values=images, output_hidden_states=True, return_dict=True)
            hidden_states = tuple(state[:, 1 + self.num_register_tokens:, :] for state in outputs.hidden_states[1:])
        else:
            # ``norm=False`` returns each post-block, pre-final-norm representation.
            # DINOv3's interface strips [CLS] and its four storage tokens by default.
            hidden_states = self.encoder.get_intermediate_layers(images, n=list(range(12)), norm=False)
        if len(hidden_states) != 12:
            raise RuntimeError(f"Expected 12 DINOv3 block outputs, got {len(hidden_states)}.")
        selected = []
        for layer_number, patch_features in zip(self.selected_layer_numbers, hidden_states):
            if patch_features.shape[1:] != (196, 768):
                raise ValueError(
                    f"Expected DINOv3 patch output (*,196,768), got {tuple(patch_features.shape)}; "
                    "use 224x224 inputs with DINOv3 ViT-B/16."
                )
            patch_features = patch_features.reshape(batch_size, num_frames, 196, 768)
            selected.append(patch_features.index_select(-1, getattr(self, f"neuron_indices_l{layer_number}")))
        return torch.cat(selected, dim=-1)

    def forward(self, x):
        batch_size, num_frames, _, height, width = x.shape
        if (height, width) != (224, 224):
            raise ValueError(f"DINOv3-NeuronDeMamba requires 224x224 input, got {height}x{width}")
        images = x.reshape(batch_size * num_frames, 3, height, width)
        features = self._selected_patch_features(images, batch_size, num_frames)
        if features.shape[-1] != 768:
            raise RuntimeError(f"Expected selected DINOv3 width 768, got {features.shape[-1]}")
        features = features.to(dtype=self.fc1.weight.dtype)
        global_feat = self.fc_norm2(features.mean(dim=(1, 2)))
        mamba_input = features.reshape(batch_size, num_frames, 1, 14, 1, 14, 768)
        mamba_input = mamba_input.permute(0, 2, 4, 1, 3, 5, 6).contiguous()
        mamba_input = mamba_input.reshape(batch_size * 196, num_frames, 768)
        local_feat = self.fc_norm(self.mamba(mamba_input).mean(dim=1).reshape(batch_size, -1))
        return self.dropout(self.fc1(torch.cat((global_feat, local_feat), dim=1)))


class DINOv2_NeuronDeMamba(nn.Module):
    """HF DINOv2 ViT-B/14 at 196px with the unchanged DeMamba head.

    A 196px input and patch size 14 produce 14x14=196 patch tokens. Selecting
    64 channels from each of DINOv2's 12 width-768 blocks therefore preserves
    the exact [B,T,196,768] interface consumed by the existing DeMamba head.
    """

    def __init__(self, neuron_indices_path, dinov2_hf_model_path, class_num=4):
        super().__init__()
        hf_model_path = os.path.abspath(dinov2_hf_model_path or "")
        if not os.path.isdir(hf_model_path):
            raise FileNotFoundError(
                "Local Hugging Face DINOv2 directory not found: "
                f"{hf_model_path}. Download facebook/dinov2-base first."
            )
        try:
            self.encoder = AutoModel.from_pretrained(hf_model_path, local_files_only=True)
        except (ImportError, KeyError, OSError, ValueError) as error:
            raise RuntimeError(
                "Unable to load Hugging Face DINOv2. Install a compatible transformers release and make sure "
        ...
        self.num_register_tokens = int(getattr(self.encoder.config, "num_register_tokens", 0))
        self.patch_nums = 196
        if (self.hidden_size, self.patch_size, self.num_hidden_layers, self.num_register_tokens) != (768, 14, 12, 0):
            raise ValueError(
                "Expected standard DINOv2 ViT-B/14 (12 blocks, width 768, patch 14, no registers), got "
                f"blocks={self.num_hidden_layers}, width={self.hidden_size}, patch={self.patch_size}, "
                f"registers={self.num_register_tokens}. Use facebook/dinov2-base, not a registers variant."
            )
        ...
        with open(selector_path, "r", encoding="utf-8") as handle:
            selector = json.load(handle)
        if selector.get("backbone") not in (None, "dinov2_vitb14"):
            raise ValueError(f"Selector was not made for DINOv2 ViT-B/14: {selector.get('backbone')!r}")
        layers = selector.get("layers", selector.get("selected_indices", selector))
        if not isinstance(layers, dict):
            raise ValueError(f"Invalid selector file: {selector_path}")
        ...
        for raw_layer, raw_indices in sorted(layers.items(), key=lambda item: self._selector_layer_number(item[0])):
            ...
            indices = torch.as_tensor(raw_indices, dtype=torch.long)
            if layer_number < 1 or layer_number > 12 or indices.ndim != 1 or indices.numel() == 0:
                raise ValueError(f"Invalid DINOv2 selector for layer {raw_layer!r}")
            if int(indices.min()) < 0 or int(indices.max()) >= 768 or indices.unique().numel() != indices.numel():
                raise ValueError(f"Invalid DINOv2 channel indices for layer {raw_layer!r}")
            self.register_buffer(f"neuron_indices_l{layer_number}", indices, persistent=True)
            self.selected_layer_numbers.append(layer_number)
            selected_width += int(indices.numel())
        if self.selected_layer_numbers != list(range(1, 13)) or selected_width != 768:
            raise ValueError("DINOv2 selector must contain 64 valid channels for each of 12 blocks (768 total).")
    ...
    def train(self, mode=True):
        """Train DINOv2 for SFT, but keep it deterministic after LP freezing."""
        super().train(mode)
        if not any(parameter.requires_grad for parameter in self.encoder.parameters()):
            self.encoder.eval()
        return self
    ...
    def _selected_patch_features(self, images, batch_size, num_frames):
        ...
        if len(hidden_states) != 12:
            raise RuntimeError(f"Expected 12 DINOv2 block outputs, got {len(hidden_states)}.")
        selected = []
        for layer_number, state in zip(self.selected_layer_numbers, hidden_states):
            patch_features = state[:, 1:, :]  # Remove only the CLS token; standard DINOv2 has no registers.
            if patch_features.shape[1:] != (196, 768):
                raise ValueError(
                    f"Expected DINOv2 patch output (*,196,768), got {tuple(patch_features.shape)}; "
                    "use 196x196 inputs with standard ViT-B/14."
                )
            patch_features = patch_features.reshape(batch_size, num_frames, 196, 768)
            selected.append(patch_features.index_select(-1, getattr(self, f"neuron_indices_l{layer_number}")))
        return torch.cat(selected, dim=-1)
    ...
    def forward(self, x):
        batch_size, num_frames, _, height, width = x.shape
        if (height, width) != (196, 196):
            raise ValueError(f"DINOv2-NeuronDeMamba requires 196x196 input, got {height}x{width}")
        images = x.reshape(batch_size * num_frames, 3, height, width)
        features = self._selected_patch_features(images, batch_size, num_frames)
        features = features.to(dtype=self.fc1.weight.dtype)
        global_feat = self.fc_norm2(features.mean(dim=(1, 2)))
