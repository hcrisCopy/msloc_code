...
def extract(task):
    ...
    result = subprocess.run(
        # The fps filter gives a deterministic output time grid.  Together
        # with -start_number 1 this makes frame_n.jpg represent
        # t=(n-1)/fps, which is the invariant checked by the frame probe.
        ["ffmpeg", "-y", "-hide_banner", "-loglevel", "error", "-i", str(source),
         "-vf", f"fps=fps={fps}:start_time=0:round=near", "-start_number", "1",
         str(destination / "frame_%d.jpg")],
